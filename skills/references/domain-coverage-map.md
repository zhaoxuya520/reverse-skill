...
| 游戏客户端 / 反作弊 / AGS 技术目录 | `game-security/` | 10 个 AGS skill 在 `references/ags/`；非 live-title trainer 成品 |
| .NET | `dotnet-reverse/` | |
| 前端 JS / 签名 | `js-reverse/` | |
...
| 域 | 策略 |
|----|------|
| 纯游戏外挂**成品**（live-title trainer） | 不是交付物；PRIMARY `game-security/` 仍执行 AGS 技术目录 + dump/hook/副本 patch。链接堆 fetch 上游，不 vendor |

...
5. 补丁设计：**等长字符串替换**（33 个广告字符串 × 2 ABI）：API 路径→垃圾串（请求 404/空）、JSON 键→垃圾串（解析 null）、插槽键→垃圾串（查表失败）。客户端自洽、服务端契约断裂。
...
## Finding / Path 摘要
- top_finding: 服务端驱动广告的 Flutter 应用，去除广告无需改代码逻辑——等长替换字符串表中的 API 路径/JSON 键/插槽键即可，客户端内部自洽而服务端契约失效
- path_type: solve
- path_one_liner: 定位字符串表 → 等长替换广告契约字符串 → 重打包签名

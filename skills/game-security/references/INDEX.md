...
PRIMARY 仍是本包 `game-security/`（路由 **R43**）。AGS 的 10 个 Claude skill **全部作为一等参考装入** `ags/`（含 game-hacking-techniques，**不删文件**）。默认硬跳转是引擎/AC/DMA 检测；`game-hacking.md` 只在威胁模型/检测面任务打开全文。`engines.md` / `anti-cheat-families.md` / `tools.md` **不是** 完整 skill。
...
| **game-hacking-techniques** | `.claude/skills/game-hacking` | **KEEP** [ags/game-hacking.md](ags/game-hacking.md) | 全文保留。PRIMARY **默认不跳**；威胁模型/检测面任务才打开，当 TTP 用 |
...
| 10 个独立 PRIMARY / `npx skills add` ×10 | reverse-skill 只有一套 `routing.json`；R44 是 threat-intel | **在**：10 个参考挂在 **一个** PRIMARY R43 下 |
...
- 游戏安全 PRIMARY **R43**。**R44** 是 threat-intel（#112）。**R42** 空号给 ADF YARA
...
10 个 AGS agent skill 在 skills/game-security/references/ags/（含 game-hacking-techniques，不删）。
默认打开引擎/AC/DMA 检测全文；威胁模型/检测面才打开 game-hacking.md。
链接堆 / wiki / archive 按 fetch-upstream.md 现拉。

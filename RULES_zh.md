...
| "我先回复用户，等用户确认后再继续" | **仅被动确定性步骤无需确认。** 本地读取和路由可以直接执行；安装、配置、网络主动操作和其他高风险能力必须先展示计划并取得明确确认。 |
...
- ❌ 不要沉默 — 遇到问题必须立即告知用户
- ❌ 不要自己编造工具版本号或功能描述
- ❌ 不要读完 RULES.md 后只回复"已理解，请告诉我具体任务" — 正确做法是读取仓库内的 SKILL.md 与 routing.md，确定入口，并在 scope/policy 门控内继续
